use geo::{coord, Contains, Distance, Point, Polygon, HaversineDistance};
use polars::prelude::*;
...
fn geo_haversine_distance(
    ...
    match (from_point, to_point) {
        (Some(from_point), Some(to_point)) => {
            // Use the correct geo crate API for haversine distance
            Some(from_point.haversine_distance(&to_point))
        }
